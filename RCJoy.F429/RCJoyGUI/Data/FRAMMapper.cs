...
//        event FRAMUserEvent AddressRequired;
    ...
    {
        private const uint VARIABLES_START = 10;
        private const uint MAX_ADDRES = 1022;

        private static readonly Dictionary<uint, IFRAMUser> __Map = new Dictionary<uint, IFRAMUser>();

        public static void Check()
        {
            if(__Map.Keys.Any(k => k > MAX_ADDRES))
                throw new CompilationCheckException("Variables memory is full, unable to create variables map", CompilationCheckException.CompileIteration.EEPMaping);
        }
