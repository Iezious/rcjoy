        ...
        private void ParseGlobal(uint tag, uint val)
        {
            switch (tag)
            {
                ...
                case TAG_GLOBAL_REPORTID:
                    throw new ArgumentException("Sorry, but partial reports is not supproted yet!");
...
/*            if (__CurrentUsageBlock == GLOBAL_USAGE_GAME_CONTROLS)
            {
...
*/
            ...
            if (__CurrentUsageBlock != GLOBAL_USAGE_GENERIC_CONTROLS && __CurrentUsageBlock != GLOBAL_USAGE_GAME_CONTROLS)
            {
                __Usages.Clear();
                return;
            }
            ...
            Func<uint, uint> GetMaxBits = (bits) =>
            {
                var res = 0U;
            ...
            Action<string, string> CreateAxle = (name, cname) => _Joystick.Controls.Add(new JoystickAxle
            {
                Length = (int) __Bits,
                MinValue = 0,
                ...
                switch (usage)
                {
                    ...
                    case HID_USAGE_HATSWITCH:
                        ...
                        _Joystick.Controls.Add(new HatSwitch
                        {
                            ...
                            MinValue = 0,
                            MaxValue = (int) GetMaxBits(__Bits),
                            ConstantName = "HAT_" + hats_counter,
                            Name = "Hat " + hats_counter
                        });
                        hats_counter++;
                        break;

                    default:
                        throw new ArgumentException("Not supported usage type");
                }
            }
