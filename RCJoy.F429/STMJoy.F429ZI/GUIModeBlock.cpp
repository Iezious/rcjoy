...
{
	Block = block;
	ListBox.Length = block->VarsCount + block->LinksCount + 1;
	...
	for(u8 i=0; i < block->LinksCount; i++)
	{
...
{
	if(lnk->LINK_BITS & LINK_AXIS)	
		BSP_LCD_SeftFontColor(COLOR_AXIS);		
	else if(lnk->LINK_BITS & LINK_VALUE)
		BSP_LCD_SetFontColor(COLOR_VARIABLE);
	else 
		BSP_LCD_SetFontColor(COLOR_BUTTON);
...
{
	BSP_LCD_SetFontColor(COLOR_NAME);
	BSP_LCD_SetFont(FONT_NAME);
	BSP_LCD_DisplatStringAt(left + FONT_PKS->Width + 4, top + 4, lnk->Name, LEFT_MODE);
}
...
static void DrawValue(u16 addr, uint16_t left, uint16_t top, uint16_t width)
{
	...
	BSP_LCD_DisplatStringAt(
		left + width - FONT_PKS->Width - 4 - FONT_VALUE * strlen(buffer), 
		top + 2, 
		__buffer, LEFT_MODE);
}
...
{
	BSP_LCD_SeftFontColor(COLOR_BUTTON);		
	BSP_LCD_SetFont(FONT_VALUE);
	
	if(CHECK_DOWN(addr))
	{
		BSP_LCD_DisplatStringAt(
			left + width - FONT_PKS->Width - 4 - FONT_VALUE * 4, 
			top + 2, "DOWN", LEFT_MODE);
	}
	else
	{
		BSP_LCD_DisplatStringAt(
...
{
	DrawItemName(lnk->Name, left, top);
	DrawLinkSymbol(lnk, left, top, width);
	
	if(lnk -> LINK_BITS & LINK_BUTTON)
	{
		DrawButon(lnk->DATAMAP_ADDR, left, top, width);
	}	
	else
	{
		BSP_LCD_SefFontColor(lnk->LINK_BITS & LINK_AXIS ? COLOR_AXIS : COLOR_VALUE);		
		DrawValue(lnk->DATAMAP_ADDR, left, top, width);
	}
}

static void DrawVarible(VariableDef* var, u16 left, u16 top, u16 width)
{
	DrawItemName(var->Name, left, top);
	BSP_LCD_SetFontColor(var->EEPROM_ADDR ? COLOR_VARIABLE : COLOR_EEPROM_VAR);
...
static void GetItem(u8 idx, u8* obj_idx, u8* isVar)
{
	...
	*isVar = 0;
	...
	else
	{
		...
		for(u8 i = 0; i < Blok->LinksCount; i++)
		{
			if(Block->Links[i].LinkBits & LINK_OUTPUT)
			{
				if(idx == 0)
				{
					*obj_idx = i;
...
{
	if (ListBox.Length != 0)
		DrawList(&BlockDataList, &ListBox);
	else
	{
