...
void StatusBox::SetModel(const char *status)
{
	SetModelName(status);
}

void StatusBox::SetFlightMode(const char *status)
{
	SetModelMode(status);
}
...
void StatusBox::SetTrimmer(uint16_t trim, int16_t value)
{
	SetTrimmerValue(trim, value);
}
#endif