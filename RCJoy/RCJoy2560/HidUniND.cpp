...
uint16_t HIDNoDelay::GetHidClassDescrLen(uint8_t type, uint8_t num) {
...
bool HIDNoDelay::SetReportParser(uint8_t id, HIDReportParser *prs) {
	for (uint8_t i = 0; i < MAX_REPORT_PARSERS; i++) {
...
HIDReportParser* HIDNoDelay::GetReportParser(uint8_t id) {
	if (!bHasReportId)
...
uint8_t HIDNoDelay::Init(uint8_t parent, uint8_t port, bool lowspeed) {
...
HIDNoDelay::HIDInterface* HIDNoDelay::FindInterface(uint8_t iface, uint8_t alt, uint8_t proto) {
...
void HIDNoDelay::EndpointXtract(uint8_t conf, uint8_t iface, uint8_t alt, uint8_t proto, const USB_ENDPOINT_DESCRIPTOR *pep) {
...
bool HIDNoDelay::BuffersIdentical(uint8_t len, uint8_t *buf1, uint8_t *buf2) {
...
void HIDNoDelay::ZeroMemory(uint8_t len, uint8_t *buf) {
	for (uint8_t i = 0; i < len; i++)
		buf[i] = 0;
}

void HIDNoDelay::SaveBuffer(uint8_t len, uint8_t *src, uint8_t *dest) {
	for (uint8_t i = 0; i < len; i++)
		dest[i] = src[i];
}

uint8_t HIDNoDelay::ForcePoll()
{
	qNextPollTime = 0;
	return 0;
}

uint8_t HIDNoDelay::Poll() {
	uint8_t rcode = 0;

	if (!bPollEnable)
		return 0;

	while (qNextPollTime <= millis()) { delay(1); }

	qNextPollTime = millis() + 5;
