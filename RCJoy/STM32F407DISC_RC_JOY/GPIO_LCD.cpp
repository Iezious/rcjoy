...
#define E_PULSE spinWait(15); GPIOB->ODR &= ~GPIO_Pin_15; spinWait(10); GPIOB->ODR |= GPIO_Pin_15
#define LCD_PORT GPIOE
#define LCD_PORT_PH RCC_AHB1Periph_GPIOE
#endif 
...
#define LCD_PORT_PH RCC_AHB1Periph_GPIOD
...
static void gpio_init()
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB | LCD_PORT_PH, ENABLE);
	...
	LCD_PORT->ODR = LCD_PORT->ODR & ~(GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_11 | GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15);
}
...
#define LCD_STATIC_MASK ~(GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_11 | GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15)

static void write4bits(uint8_t data, uint8_t isdata = 0)
{
	u16 pdata = data & 0xF;
	LCD_PORT->ODR = GPIOD->ODR & LCD_STATIC_MASK | (pdata << 12) | (isdata ? GPIO_Pin_9 : 0);
	E_PULSE;
	LCD_PORT->ODR = GPIOD->ODR & LCD_STATIC_MASK;
	spinWait(43);
}

static void gpio_send(uint8_t data, uint8_t isdata)
{
	write4bits(data >> 4, isdata);
	write4bits(data, isdata);
}
...
void GPIO_LCD::init(uint8_t lcd_cols,uint8_t lcd_rows)
{
	_cols = lcd_cols;
	_rows = lcd_rows;
